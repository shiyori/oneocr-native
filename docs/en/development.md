...
Pushing a version tag matching `version.json` (for example `v0.1.1`) automatically runs `release-publish`: build Android full/Core AARs, the universal Python distribution and one complete Linux package per architecture; verify relocated offline Linux OCR and installation from official dependencies; audit contents, licenses and checksums; upload a draft and verify remote SHA-256 digests; then publish a stable release and mark it Latest. Build records bind every platform's artifacts to the same source commit, and regular CI must pass. Download documentation defaults to `releases/latest` and `latest/download`.
