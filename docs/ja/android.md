...
[oneocr-android.aar](https://github.com/shiyori/oneocr-native/releases/latest/download/oneocr-android.aar) をアプリモジュールの `libs` ディレクトリへ置き、同モジュールの `build.gradle.kts` に追加します。
...
アプリでは OneOCR AAR を一種類だけ使用してください。[既定モデル](https://github.com/shiyori/oneocr-native/releases/latest/download/oneocr-cjk-en.ocrpack)は個別にも取得できます。[ランタイム互換性](runtime.md)も参照してください。
