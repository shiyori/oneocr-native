...
[oneocr-android-0.1.0.aar](https://github.com/shiyori/oneocr-native/releases/latest/download/oneocr-android-0.1.0.aar) をアプリモジュールの `libs` ディレクトリへ置き、同モジュールの `build.gradle.kts` に追加します。
...
ホスト ORT がある場合は [Core AAR](https://github.com/shiyori/oneocr-native/releases/latest/download/oneocr-android-core-0.1.0.aar) を使います。OneOCR Java/JNI のみを含み、モデルと ORT は含みません。既定モデルを `src/main/assets/oneocr-cjk-en.ocrpack` に保存するか、Go で導入したコマンドでアプリを準備します。
...
アプリでは OneOCR AAR を一種類だけ使用してください。[既定モデル](https://github.com/shiyori/oneocr-native/releases/latest/download/oneocr-model-cjk-en-0.1.0.ocrpack)は個別にも取得できます。[ランタイム互換性](runtime.md)も参照してください。
